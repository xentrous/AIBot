...
AI_Bot = chatterbot.ChatBot("Scofield",
...
class Client(object):
    ...
    def __init__(self, email, password, debug=True, user_agent=None , max_retries=5):
        ...
        self._console("\nAquiring Authentication.")

        for i in range(1,max_retries+1):
            if not self.login():
                self._console("\nAttempt #{} Failed{}".format(i,{True:', Retrying'}.get(i<5,'')))
                time.sleep(1)
                continue
            else:
                self._console("\nLogin Successful.")
    ...
    def whois(self):
        ...
        try:
            ...
            sent = self.send(self.fbid, answer)
            if sent:
                print("\n" + "Sent: " + str(answer))
        except:
            self.trained()
    ...
    def trained(self):
        answer=AI_Bot.get_response(self.ques)
        sent = self.send(self.fbid, answer)
        if sent:
            print("\n" + "Sent: " + str(answer))
    ...
    def on_message(self, mid, author_id, author_name, message, metadata):
        ...
        if str(self.uid)!=str(author_id):

            print("\nRecieved: %s"%(message))

            self.ques=message.strip()
            self.ques=self.ques.lower()
            self.fbid=author_id
            
            if self.ques=='bye':
                sent = self.send(author_id, "Hoping that you enjoyed having this conversation.")
                if sent:
                    print("\nSent: Hoping that you enjoyed having this conversation.")
                sent = self.send(self.fbid, "Bbye...")
                if sent:
                    print("Sent: Bbye...")

            elif 'who' in self.ques:
                if 'you' in self.ques:
                    self.trained()
                else:
                    self.whois()
            elif 'what' in self.ques:
                if 'name' in self.ques:
                    self.trained()
                else:
                    self.whatis()
            else:
